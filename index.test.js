...
tape('short and long options', t => {
	...
	t.deepEqual(opsh($`--hello=`), [
		{ option: 'hello', type: 'option', value: '' }
	]);
	t.deepEqual(opsh($`--hello=world`), [
...
tape('long options', t => {
	t.deepEqual(opsh($`--my-var=my-value`), [
		{ type: 'option', option: 'my-var', value: 'my-value' }
	]);

	t.deepEqual(opsh($`--env.target=production`), [
...
tape('operands', t => {
	t.deepEqual(opsh($`--hello file.txt`), [
		{ option: 'hello', type: 'option' },
		{ operand: 'file.txt', type: 'operand', option: 'hello' }
	]);
	t.deepEqual(opsh($`--hello=world file.txt`), [
		{ option: 'hello', type: 'option', value: 'world' },
